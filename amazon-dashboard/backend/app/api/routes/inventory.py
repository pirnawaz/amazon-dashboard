...
def _inventory_item_response(row: InventoryLevel) -> InventoryItemResponse:
    ...
    # Phase 11.4: use as_of_at when present else updated_at for hour-based freshness
    ts = row.as_of_at if row.as_of_at is not None else row.updated_at
    freshness_status, age_hours = freshness_from_timestamp(ts)
    as_of_str = row.as_of_at.isoformat() if row.as_of_at is not None else None
    return InventoryItemResponse(
        ...
        as_of_at=as_of_str,
        inventory_freshness=freshness_status,
        inventory_age_hours=age_hours,
    )
...
def restock(
    ...
    # D) On-hand per SKU: Phase 11.5 prefer inventory_levels when marketplace set, else legacy inventory_snapshots
    latest_subq = (
        select(
    ...
    # Union of SKUs: those with sales in period or those with inventory (snapshots)
    all_skus = set(total_units_by_sku.keys()) | set(on_hand_by_sku.keys())
    # Phase 11.5: when marketplace != "ALL", overwrite on_hand with inventory_levels "best" row per (sku, marketplace); fallback stays snapshot
    source_by_sku: dict[str, str | None] = {}
    if marketplace != "ALL":
        for sku in all_skus:
            on_hand_val, inv_source, _ = get_on_hand_for_restock(db, sku, marketplace)
            if inv_source is not None:
                on_hand_by_sku[sku] = int(on_hand_val)
                source_by_sku[sku] = inv_source
            else:
                if sku not in on_hand_by_sku:
                    on_hand_by_sku[sku] = 0
                source_by_sku[sku] = None  # legacy snapshot or no data
    for sku in all_skus:
        if sku not in source_by_sku:
            source_by_sku[sku] = None
    if not all_skus:
    ...
    for sku in all_skus:
        ...
        rows.append(
            RestockRow(
                ...
                inventory_source=source_by_sku.get(sku),
            )
        )
