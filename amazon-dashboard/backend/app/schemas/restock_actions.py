"""Pydantic schemas for Restock Actions API (Phase 5C). Phase 11.4: inventory freshness. Phase 12.3: data_quality."""
...
# Phase 11.4: freshness status for restock response
RestockInventoryFreshness = Literal["unknown", "fresh", "warning", "critical"]
...
class RestockActionItem(BaseModel):
    ...
    # Phase 11.4: inventory freshness for this SKU+marketplace (when inventory used)
    inventory_freshness: RestockInventoryFreshness | None = Field(
    ...
    inventory_as_of_at: datetime | None = Field(None, description="Last inventory timestamp used")
    inventory_warning_message: str | None = Field(
        None, description="Human-friendly warning when stale (warning/critical)"
    )
...
class RestockActionsResponse(BaseModel):
    ...
    # Phase 12.3: demand data quality (marketplace summary)
